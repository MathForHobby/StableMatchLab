...
def render_game():
    ...
    top1, top2, top3, top4, top5 = st.columns([1.0, 1.05, 1.25, 0.85, 1.25])
    with top1:
    ...
    with top4:
        n = st.selectbox(
            "인원",
            [3, 4, 5, 6],
            index=[3, 4, 5, 6].index(st.session_state.n),
            label_visibility="collapsed",
            key="people_count_selector",
        )
...
def render_sidebar_navigation():
    with st.sidebar:
        ...
        st.markdown(
            '<div class="sidebar-note">게임 중 언제든지 첫 화면으로 돌아갈 수 있습니다.</div>',
            unsafe_allow_html=True,
        )
        st.write("")
        ...
        st.write(f"인원: {st.session_state.n}쌍")
